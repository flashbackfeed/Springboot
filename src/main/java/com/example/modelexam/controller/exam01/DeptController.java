 ...
 * 요약 :
 *        @Slf4j : 로깅 라이브러리 (롬복) , 인터페이스 : 자식클래스(logback)
 *        사용법 : 예) 1) @Slf4j 클래스 위에 달고
 *                     2_ 로깅하고 싶은 라인에서 log.debug(값.toString());
 ...
 */
// TODO : @Slf4j - 롬복 , 로깅 라이브러리를 위한 어노테이션 ( 디자인패턴 : Facade 패턴 )
//        로깅 라이브러리 : log4j -> log4j2 , logback -> 함수호출을 일원화 시켜주는 기능
//     사용법 : 클래스 위에 붙이면 사용 가능 : log.debug(값)
